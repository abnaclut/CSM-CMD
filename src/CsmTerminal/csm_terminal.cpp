...
Terminal::Terminal(std::chrono::milliseconds command_timeout)
...
int Terminal::executeWithTimeout(const std::string& name, const std::vector<std::string>& args)
{
...
int Terminal::dispatch(const std::vector<std::string>& tokens)
{
  if (tokens.empty())
  {
    return 0;
